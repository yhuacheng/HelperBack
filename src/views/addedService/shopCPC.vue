<template>
	<section>
		 ...
		 id="tableData" ref='tableData'>
			...
			<el-table-column prop="ShopCpcType" label="报告截图" align="center">
				<template slot-scope="scope">
					<img style="width: 40px;height: 40px;" v-show="scope.row.ShopCpcType" :src="scope.row.ShopCpcType" @click.stop="showImage(scope.$index,scope.row)" />
				</template>
			</el-table-column>
		...
		<!-- 导入 -->
		<el-dialog title="【店铺CPC报告】文件导入" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal"
		 width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag action="/api/BackShopCpcReport/ExcelShopCpcReport" multiple :on-success="handleAvatarSuccess"
		...
		<!-- 上传图片 -->
		<el-dialog title="【店铺CPC报告】上传报告截图" :visible.sync="drImgModal" :close-on-click-modal="false" :before-close="closeDrImgModal"
		 width="30%">
			<div class="txt-c">
	...
	export default {
		...
		methods: {
			...
			showImage(index, row) {
				...
				_this.bigImageUrl = row.ShopCpcType
			},
