<template>
	<section>
		...
		<!-- 导入 -->
		<el-dialog title="【关键词上首页】文件导入" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal"
		 width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag action="/api/BackKeyWordUpPag/ExcelKeyWordUpPag" multiple :on-success="handleAvatarSuccess"
		...
		methods: {
			...
			handleMod(val) {
				...
				if (val == 1) {
					...
					keyToIndexState(params).then(res => {
						_this.getData()
					}).catch((e) => {})
				}
				if (val == 2) {
