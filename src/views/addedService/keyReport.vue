<template>
	<section>
		 ...
		 id="tableData" ref='tableData'>
			...
			<el-table-column prop="AsinType" label="报告截图" align="center">
				<template slot-scope="scope">
					<img style="width: 40px;height: 40px;" v-show="scope.row.AsinType" :src="scope.row.AsinType" @click.stop="showImage(scope.$index,scope.row)" />
				</template>
			</el-table-column>
		...
		<!-- 导入 -->
		<el-dialog title="【ASIN关键词报告】文件导入" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal"
		 width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag action="/api/BackAsinKeyWord/ExcelAsinKeyWord" multiple :on-success="handleAvatarSuccess"
		...
		<!-- 上传图片 -->
		<el-dialog title="【ASIN关键词报告】上传报告截图" :visible.sync="drImgModal" :close-on-click-modal="false" :before-close="closeDrImgModal"
		 width="30%">
			<div class="txt-c">
		...
		methods: {
			...
			handleMod(val) {
				...
				if (val == 1) {
					...
					keyReportState(params).then(res => {
						_this.getData()
					}).catch((e) => {})
				}
				if (val == 2) {
			...
			showImage(index, row) {
				...
				_this.bigImageUrl = row.AsinType
			},
