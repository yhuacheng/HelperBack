<template>
	<section>
		 ...
		 id="tableData" ref='tableData'>
			...
			<el-table-column label="截图" align="center">
				<template v-slot="scope">
					<el-link type="primary" :underline="false" :href="scope.row.Images" v-if="scope.row.Images">
						{{scope.row.Images}} <i class="el-icon-download"></i>
					</el-link>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" align="center">
				<template v-slot="scope">
					...
					<el-button size="small" type="primary" @click.stop="handleUpload(scope.$index, scope.row)" v-if="scope.row.State===3">上传截图</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<!-- 导入 -->
		<el-dialog title="【加购】上传截图" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal" width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag name="image" action="/api/BackAddShopCart/UserByAddShopCartPictureUpload"
				 multiple :on-success="handleAvatarSuccess" :on-error="handleError">
