<template>
	<section>
		 ...
		 id="tableData" ref='tableData'>
			...
			<el-table-column prop="ShopType" label="报告截图" align="center">
				<template slot-scope="scope">
					<img style="width: 40px;height: 40px;" v-show="scope.row.ShopType" :src="scope.row.ShopType" @click.stop="showImage(scope.$index,scope.row)" />
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" align="center" width="100">
				<template v-slot="scope">
					<el-button size="small" type="primary" v-if="scope.row.State==3"  @click.stop="handleUploadImg(scope.$index, scope.row)">上传截图</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<!-- 导入 -->
		<el-dialog title="【店铺体检报告】文件导入" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal"
		 width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag action="/api/BackShopReport/ExcelShopReport" multiple :on-success="handleAvatarSuccess"
		...
		<!-- 上传图片 -->
		<el-dialog title="【店铺体检报告】上传报告截图" :visible.sync="drImgModal" :close-on-click-modal="false" :before-close="closeDrImgModal"
		 width="30%">
			<div class="txt-c">
		...
		<!--查看图片大图-->
		<el-dialog title="查看大图" :visible.sync='imageModal' :close-on-click-modal='false' :before-close="closeImageModal">
			<div class="txt-c">
		...
		methods: {
			...
			handleMod(val) {
				...
				if (val == 1) {
					...
					shopCheckState(params).then(res => {
						_this.getData()
					}).catch((e) => {})
				}
				if (val == 2) {
			...
			showImage(index, row) {
				...
				_this.bigImageUrl = row.ShopType
			},
