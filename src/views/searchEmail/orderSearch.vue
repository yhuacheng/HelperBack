<template>
	<section>
		...
		<!-- 导入 -->
		<el-dialog title="【订单查邮箱】文件导入" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal"
		 width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag action="/api/BackOrderMailboxQuery/ExcelUserByOrderMailboxQuery" multiple
		...
		methods: {
			...
			handleMod(val) {
				...
				if (val == 1) {
					...
					orderSearchState(params).then(res => {
						_this.getData()
					}).catch((e) => {})
				}
				if (val == 2) {
