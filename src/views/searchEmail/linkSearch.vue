	...
	<section>
		...
		<el-table :data="tableData" @selection-change="selsChange" @row-click="rowClick" v-loading="listLoading" style="width: 100%"
		 id="tableData" ref='tableData'>
			...
			<el-table-column prop="ReviewLink" label="Review" align="center" width="200"></el-table-column>
		...
		<el-table v-show="false" :data="tableData" style="width: 100%" id="out-table">
			...
			<el-table-column prop="Grade" label="星级" align="center"></el-table-column>
			<el-table-column prop="Comments" label="评论内容" align="center"></el-table-column>
			<el-table-column prop="BuyerName" label="买家姓名" align="center"></el-table-column>
			<el-table-column prop="BuyerEmail" label="买家邮箱" align="center"></el-table-column>
		...
		<!-- 导入 -->
		<el-dialog title="【链接查邮箱】文件导入" :visible.sync="drModal" :close-on-click-modal="false" :before-close="closeDrModal"
		 width="30%">
			<el-form ref="drForm" class="txt-c">
				<el-upload ref="upload" :limit="1" drag action="/api/BackLinkMailboxQuery/ExcelUserByLinkMailboxQuery" multiple
		...
		methods: {
			...
			handleMod(val) {
				...
				if (val == 1) {
					...
					linkSearchState(params).then(res => {
						_this.getData()
					}).catch((e) => {})
				}
				if (val == 2) {
